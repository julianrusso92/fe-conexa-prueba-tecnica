...
export default async function Page({
  ...
  const promisePeople: Promise<People> = fetchWithSearch({
    page,
    search,
    resource: "people",
  });
    ...
    <section className="py-8">
      <div className="container">
        <div className="flex flex-col md:flex-row items-center justify-between md:gap-y-0 gap-y-2 md:gap-x-16 px-2">
          ...
          <div className="grow">
            <SearchBar search={search} resource="people" />
          </div>
          <div className="flex space-x-6">
            <Await promise={promisePeople}>
              {(data) => (
                <>
                  <Link
                    href={{
            ...
            <div className="flex flex-wrap gap-0">
              {data.results.map((person) => (
                <Card key={person.name}>
                  ...
                  <div className="font-light">Gender: {person.gender}</div>
                  <Link
